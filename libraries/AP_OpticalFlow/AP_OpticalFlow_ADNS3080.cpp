...
// We use hal.console->Port 2 in SPI Mode
...
bool
AP_OpticalFlow_ADNS3080::init(bool initCommAPI, AP_Semaphore* spi_semaphore, AP_Semaphore* spi3_semaphore)
{
    int8_t retry = 0;
    bool retvalue = false;
...
AP_OpticalFlow_ADNS3080::read_register(uint8_t address)
{
    ...
    if( _spi_bus == ADNS3080_SPIBUS_1 ) {
        ...
        hal.scheduler->delayMicroseconds(50);          // small delay
...
AP_OpticalFlow_ADNS3080::reset()
{
    ...
    hal.scheduler->delayMicroseconds(10);
...
AP_OpticalFlow_ADNS3080::print_pixel_data()
{
    ...
    hal.scheduler->delayMicroseconds(1510);      // min frame speed is 2000 frames/second so 1 frame = 500 nano seconds.  so 500 x 3 + 10 = 1510
    ...
    for( i=0; i<ADNS3080_PIXELS_Y; i++ ) {
        for( j=0; j<ADNS3080_PIXELS_X; j++ ) {
            ...
            if( isFirstPixel && (regValue & 0x40) == 0 ) {
                hal.console->print_P(PSTR("failed to find first pixel"));
            }
            isFirstPixel = false;
            ...
            hal.scheduler->delayMicroseconds(50);
        }
        hal.console->println();
    }
